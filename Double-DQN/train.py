        ...
        if len(memory) >= config['mini_batch_size']:
            ...
            for state, action, new_state, reward, done in batch:
                if done:
                    ...
                    # Double DQN update
                    with torch.no_grad():
                        next_action = policy_net(one_hot_encode(new_state, num_states)).argmax().item() # use of the policy network to compute the best action
                        target_value = target_net(one_hot_encode(new_state, num_states))[next_action] #use of the target network to comput the target value
                        target = reward + config['discount_factor'] * target_value
